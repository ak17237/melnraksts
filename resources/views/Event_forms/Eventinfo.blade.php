@extends('welcome')
@section('PageTitle',$myevent->Title)
@section('content')

<div class="container">
    @if(Auth::check() && Auth::user()->hasRole('Admin') && !checkExpired($myevent->id) && checkAuthor(Auth::user()->email,$myevent->id))
    <a href="{{ route('showedit',$myevent->id) }}" class="btn btn-primary back">Rediģēt pasākumu</a>
    @endif
    <br>
    ...
    <div class="row">
        <div class="col-lg-offset-3 col-lg-11 center">
            <div class="col-lg-12" style="height: 56px;">
                @if(Auth::check() && Auth::user()->hasRole('Admin') && !checkExpired($myevent->id))
            ...
            <div>
                ...
                @endforeach
                @if(Auth::check() && $myevent->VIP != 1 && $myevent->Melnraksts === 0 && date('Y-m-d') < $myevent->Datefrom) 
                {{-- Rādīt pogu tikai ja: Lietotājs autorizēts,pasākums nav VIP,pasākums nav melnrakstā,pasākums nav beidzies --}}
